...
coef_log <- coef(summary(m_cont))["Water_g", ]
coef_lm <- coef(summary(lm_cont))["Water_g", ]

#Calculate results (per 100g increase)
or <- exp(coef_log[1] * 100)
or_lower <- exp((coef_log[1] - 1.96 * coef_log[2]) * 100)
or_upper <- exp((coef_log[1] + 1.96 * coef_log[2]) * 100)

beta <- coef_lm[1] * 100
beta_lower <- (coef_lm[1] - 1.96 * coef_lm[2]) * 100
beta_upper <- (coef_lm[1] + 1.96 * coef_lm[2]) * 100

#Create results table
results <- data.frame(
  Outcome = c("Diabetes (OR per 100g)", "HbA1c (Beta per 100g)"),
  Estimate = sprintf("%.4f (%.4f-%.4f)", 
                     c(or, beta), 
                     c(or_lower, beta_lower), 
                     c(or_upper, beta_upper)),
  P = c(coef_log[4], coef_lm[4])
)

results$P <- ifelse(results$P < 0.001, "<0.001", sprintf("%.3f", results$P))
...
#Save to CSV
write.csv(results, "Sensitivity_Analysis.csv", row.names = FALSE)
```
...
# Figure 4: Forest plot - OR by quartiles
...
# Figure 5: Forest plot - Beta by quartiles
